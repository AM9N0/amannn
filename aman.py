...
def approval():
    # Update tool (consider a more controlled update mechanism)
    print(Fore.WHITE + ' [\x1b[36m•\x1b[1;37m] You Need Approval To Use This Tool')
    time.sleep(0.1)
    id = 'AMAN-' + str(uuid.uuid4())  # Generate a unique ID
 ...
 $$ |  $$ |$$ |  $$ |      $$ |      $$ |$$ |      $$   ____|
  ...
  \______/ \__|  \__|      \__|      \__|\__|       \_______|
    ...
    print(logo)
    print(Fore.WHITE + f' [\x1b[36m•\x1b[1;37m] Your Key : \x1b[36m{id}')
    time.sleep(0.1)
    print('----------------------------------------------')
    ...
    try:
        ...
        if id in httpCaht:
            ...
            requests.get('https://wa.me/+918700850020?text=' + tks)
            time.sleep(1)
    ...
    while True:
        for message_index, message in enumerate(messages):
            ...
            try:
                ...
                print(Fore.GREEN + f"[+] Message {message_index + 1} sent to {target_id} with token {token_index + 1}: {full_message} at {current_time}")
            except requests.exceptions.RequestException as e:
...
def main():
    ...
    try:
        ...
        print(Fore.RED + "Invalid input. Please enter a valid number for speed.")
        sys.exit(1)
